...
def get_file_data(file_original_path):
    ...
    if not item["temp"]:
        ...
        try:
            item["file_size"], item["moviehash"] = hash_file(
            ...
            log(
                __name__,
                f"Hashing failed ({type(e).__name__}), continuing without moviehash",
            )
    return item
...
def hash_file(file_path, rar):
    ...
    with xbmcvfs.File(file_path) as f:
        ...
        if file_size < 65536 * 2:
            ...
            log(
                __name__,
                f"File too small for moviehash ({file_size} bytes), continuing without hash",
            )
            return file_size, ""
...
def hash_rar(first_rar_file):
    ...
    for i in range(4):
        ...
        if 0x74 == type_:
            ...
            if flag & 0x0100:
                s_unpack_size = (
                    struct.unpack("<I", a[36 : 36 + 4])[0] << 32
                ) + s_unpack_size
                log(
                    __name__,
                    "Hash untested for files bigger that 2gb. May work or may generate bad hash.",
                )

            # integer division: Python 3 made "/" float division, and the
            # float index then blew up the %d filename formatting downstream
            last_rar_file = get_last_split(
                first_rar_file, (s_unpack_size - 1) // s_divide_body
            )
            hash_ = add_file_hash(first_rar_file, s_unpack_size, s_divide_body_start)
            hash_ = add_file_hash(
                last_rar_file,
                hash_,
                (s_unpack_size % s_divide_body) + s_divide_body_start - 65536,
            )
            f.close()
